 ...
 *
 *   npm run bench            # normal, hard × 6시드 × 1000핸드 (약 70초)
 *   node test/bench.js hard 4 600
 *
...
diffs.forEach(function (d) {
  const rs = [];
  const t0 = Date.now();
  for (let s = 1; s <= nSeeds; s++) rs.push(B.benchmark({ difficulty: d, hands: hands, seed: s * 1000 + 7 }).bb100);
  ...
  const sd = rs.length > 1
    ...
    : 0;
  console.log(
    d.padEnd(7) + (mean >= 0 ? '+' : '') + mean.toFixed(1) + ' bb/100 vs TAG' +
