 ...
 *   node test/bench.js hard 6 1000 4 lag,station,rock,balanced   # 상대 성향
 *
...
const styles = process.argv[6] ? process.argv[6].split(',') : ['tag'];

styles.forEach(function (style) { diffs.forEach(function (d) {
  const rs = [];
  const t0 = Date.now();
  for (let s = 1; s <= nSeeds; s++) rs.push(B.benchmark({ difficulty: d, hands: hands, seed: s * 1000 + 7, players: players, style: style }).bb100);
  ...
  const sd = rs.length > 1
    ...
    : 0;
  console.log(
    d.padEnd(7) + (mean >= 0 ? '+' : '') + mean.toFixed(1) + ' bb/100 vs ' + style.toUpperCase() + ' (' + players + '인)' +
