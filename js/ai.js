...
(function (global) {
  ...
  const TUNE = {
    ...
    adaptiveHands: 30,
    /* 루즈해도 어그레시브(LAG)하면 전환하지 않는 문 — PFR/VPIP 가 기준 이상이면 어그레시브.
       인원이 많을수록 뒤에서 콜로 들어오는 일이 늘어 비율이 낮아지므로 VPIP 처럼 한 명당 낮춘다
       (0.78 이면 4인 0.78, 9인 0.70). 봇 측정(4인 / 9인 좌석별): LAG 0.85~0.87 / 0.71~0.85 ·
       밸런스드 0.65~0.67 / 0.56~0.73 · 스테이션 0.20 / 0.15.
       기본은 꺼 둔다(0): LAG 에게 보이던 손해(+50 → +41)는 시드 집합을 바꾼 것만으로 생기는
       차이(+50 → +44)와 같은 크기의 잡음이었고, 문을 달아도 LAG +43 · 밸런스드 +35 로 이득이
       없었다. README 튜닝 기록 참고. */
    adaptiveAggRatio: 0,
